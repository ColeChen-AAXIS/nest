            ...
            <tbody>
              {orderList.map((order, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{order.shipDate ? <TextFormat type="date" value={order.shipDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{order.status}</td>
                  <td>{order.complete ? 'true' : 'false'}</td>
                  <td>{order.petId ? <Link to={`pet/${order.petId.id}`}>{order.petId.id}</Link> : ''}</td>
