...
export interface IUrlProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const Url = (props: IUrlProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);
  ...
  const { urlList, match, loading } = props;
  return (
    <div>
            ...
            <tbody>
              {urlList.map((url, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{url.name}</td>
                  <td>{url.pet ? <Link to={`pet/${url.pet.id}`}>{url.pet.id}</Link> : ''}</td>
